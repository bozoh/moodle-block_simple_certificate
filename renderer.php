...
class block_simple_certificate_renderer extends plugin_renderer_base {
     ...
     * @return string
     */
    public function simple_certificate_tree($certs, $all=false, $showcourse=false) {
    ...
    public function render_simple_certificate_tree(simple_certificate_tree $tree) {
        $module = array('name'=>'block_simple_certificate', 'fullpath'=>'/blocks/simple_certificate/module.js', 'requires'=>array('yui2-treeview'));
        if (empty($tree->node['courses'])) {
            $html = $this->output->box(get_string('nocertificate','block_simple_certificate'));
        } else {
            $htmlid = 'simple_certificate_tree_'.uniqid();
        ...
        foreach ($node['courses'] as $course) {
            ...
            foreach ($course['certs'] as $cert) {
                ...
                if ($fs->file_exists_by_hash($cert->pathnamehash)){
                    ...
                    $result .= '<li yuiConfig=\''.json_encode($yuiconfig).'\'><div>'.html_writer::link($url, $image.$certname).'</div></li>';
                } else {
                    $result .= '<li yuiConfig=\''.json_encode($yuiconfig).'\'><div>'.html_writer::label($image.$certname).'</div></li>';
                }
            }
            if ($tree->showcourse) {
                $result.='</ul></li>';
