...
class block_simple_certificate_renderer extends plugin_renderer_base {
    ...
    public function render_simple_certificate_tree(simple_certificate_tree $tree) {
        $module = array(
            'name' => 'block_simple_certificate',
            'fullpath' => '/blocks/simple_certificate/module.js',
        ...
        if (empty($tree->node['courses'])) {
            ...
            $this->page->requires->js_init_call(
                'M.block_simple_certificate.init_tree', array(false, $htmlid)
            );
            $html = '<div id="' . $htmlid . '">';
            ...
            foreach ($course['certs'] as $cert) {
                ...
                if ($fs->file_exists_by_hash($cert->pathnamehash)) {
                    ...
                    $result .= '<li yuiConfig=\'' . json_encode($yuiconfig) . '\'><div>' .
                        html_writer::link($url, $image . $certname) . '</div></li>';
                } else {
                    $result .= '<li yuiConfig=\'' . json_encode($yuiconfig) . '\'><div>' .
                        html_writer::label($url, $image . $certname) . '</div></li>';
                }
            }
            if ($tree->showcourse) {
                $result .= '</ul></li>';
...
class simple_certificate_tree implements renderable {
    ...
    public function get_certs_tree($certs) {
        global $DB;
        ...
        foreach ($certs as $issuecert) {
            ...
            $coursename = $str[0];
            $certname = $str[1];
            $cert = $DB->get_record('simplecertificate', array('id' => $issuecert->certificateid));
